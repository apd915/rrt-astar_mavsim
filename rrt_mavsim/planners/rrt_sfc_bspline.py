...
import time
import scipy as sp
from rrt_mavsim.tools.intersections import (
    intersectionOccurred_Matrix,
...
class RRT_SFC_BSpline:
    ...
    def generateControlPoints(
        ...
        outputControlPoints = self.bsplineGen.generatePath(
            waypoints=waypoints, numPointsPerUnit=numPointsPerUnit
        )
    ...
    def __generatePaths_2D(self):
        ...
        self.tree.add(
            position=startPosition_2D, parent=np.inf, cost=0.0, connectsToGoal=False
        )
    ...
    def __generatePaths_3D(self):
        ...
        self.tree.add(
            position=self.startPosition_3D,
            parent=np.inf,
            cost=0.0,
    ...
    def __extendInitialTree(self, endPosition: np.ndarray):
        ...
        while connectedToEnd is False:
            ...
            # candidate sfc
            sfc_candidate = MsgFlightCorridor(
                primaryPosition=parentPosition,
            ...
            if (intersectionHappened is False) and satisfiesTurnConstraints:
                ...
                if newNode_toEnd_distance < self.segmentLength:
                    ...
                    sfc_candidate_newToEnd = MsgFlightCorridor(
                        primaryPosition=newPositionCandidate,
    ...
    def setWaypointsNotSmooth(self, waypoints_not_smooth: MsgWaypoints_SFC):
        self.waypoints_not_smooth = waypoints_not_smooth

    def setWaypointsSmooth(self, waypoints_smooth: MsgWaypoints_SFC):
...
def generateRandomCandidate(
    ...
    distance_vectors_list = allTreePositions_array - np.tile(
        randomPosition, (1, tree.numPositions)
    )
